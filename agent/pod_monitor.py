...
class PodMonitor:
    ...
    async def _handle_failed_pod(self, pod):
        ...
        try:
            ...
            # Send to backend
            await self.backend_client.report_failed_pod(pod_data)

            # Mark as reported
            pod_key = f"{pod.metadata.namespace}/{pod.metadata.name}"
            self.reported_pods[pod_key] = datetime.now()

            logger.info(f"Reported failed pod: {pod_key}")

        except Exception as e:
            logger.error(f"Error handling failed pod {pod.metadata.name}: {e}")
