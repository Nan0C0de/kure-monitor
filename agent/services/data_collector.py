...
def _resolve_k8s_api_timeout() -> int:
    """Resolve K8S_API_TIMEOUT_SECONDS env var, defaulting to 30s."""
    raw = os.environ.get("K8S_API_TIMEOUT_SECONDS")
    if not raw:
        return 30
    try:
        v = int(raw)
...
# Generic per-call Kubernetes API timeout (seconds). Wraps sync calls offloaded
# to the default executor so a hung apiserver call can't stall the event loop
# indefinitely. Read once at module init.
K8S_API_TIMEOUT_SECONDS = _resolve_k8s_api_timeout()

# Env-var name patterns whose VALUES must be redacted before shipping a pod
# manifest to the backend. Match is case-insensitive substring on the env-var
# NAME. Keep this aligned with the redaction unit tests.
...
# Annotation-key patterns whose VALUES must be redacted.
_SECRET_ANNOTATION_PATTERNS = ("secret", "password", "token")
_VAULT_INJECT_PREFIX = "vault.hashicorp.com/agent-inject-secret-"

_REDACTED = "[REDACTED]"


def _redact_env_value(name: str, value: str) -> str:
    """Return [REDACTED] if env-var NAME indicates a secret, else value unchanged."""
    if not isinstance(name, str) or not name:
        return value
    lowered = name.lower()
    for pat in _SECRET_ENV_NAME_PATTERNS:
        if pat in lowered:
            return _REDACTED
    return value
...
def _redact_annotation_value(key: str, value: str) -> str:
    """Return [REDACTED] if annotation KEY indicates a secret, else value unchanged."""
    if not isinstance(key, str) or not key:
        return value
    lowered = key.lower()
    if lowered.startswith(_VAULT_INJECT_PREFIX):
        return _REDACTED
    for pat in _SECRET_ANNOTATION_PATTERNS:
        if pat in lowered:
            return _REDACTED
    return value
...
class DataCollector:
    ...
    async def collect_pod_data(self, pod, v1_client) -> Dict[str, Any]:
        ...
        if self.config is not None and getattr(
            self.config, "failure_logs_enabled", False
        ):
            try:
    ...
    async def _get_pod_events(self, v1_client, namespace: str, pod_name: str) -> list:
        ...
        try:
            ...
            def _do_fetch():
                return v1_client.list_namespaced_event(
                    namespace=namespace,
            ...
            logger.warning(
                f"Timeout fetching events for pod {namespace}/{pod_name} "
                f"after {K8S_API_TIMEOUT_SECONDS}s"
            )
            return []
        except Exception as e:
    ...
    async def _get_pod_logs(self, v1_client, namespace: str, pod_name: str) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Timeout fetching logs for pod {namespace}/{pod_name} "
                f"after {K8S_API_TIMEOUT_SECONDS}s"
            )
            return ""
        except Exception as e:
    ...
    def _redact_manifest_secrets(self, pod_dict: dict) -> None:
        """In-place redact secret-bearing env-var values and annotation values.

        Env-var values are redacted when the env-var NAME matches one of the
        well-known secret patterns (password, secret, token, api_key, etc).
        Annotation values are redacted for vault-inject-secret-* keys and any
        annotation key containing secret/password/token. Mutates pod_dict.
        """
        # Redact annotations on the pod metadata.
        ...
        if isinstance(annotations, dict):
            for k in list(annotations.keys()):
                v = annotations.get(k)
                if isinstance(v, str):
                    annotations[k] = _redact_annotation_value(k, v)

        # Redact env-var values on every container (regular + init).
        spec = pod_dict.get("spec") or {}
        for container_list_key in ("containers", "init_containers", "initContainers"):
            containers = spec.get(container_list_key)
            ...
            for container in containers:
                ...
                for entry in env:
                    if not isinstance(entry, dict):
                        continue
                    name = entry.get("name")
                    if "value" in entry and isinstance(entry.get("value"), str):
                        entry["value"] = _redact_env_value(name, entry["value"])
    ...
    def _get_pod_manifest(self, pod) -> str:
        ...
        try:
            ...
            # Redact secrets BEFORE serialization so they never reach the wire.
            self._redact_manifest_secrets(pod_dict)
            ...
            manifest = yaml.safe_dump(
                clean_pod_dict, default_flow_style=False, sort_keys=False
            )
            logger.info(
                f"Generated complete manifest length: {len(manifest)} characters"
            )
            return manifest
        except Exception as e:
