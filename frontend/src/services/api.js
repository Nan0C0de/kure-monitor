...
export const api = {
  ...
  // Kyverno Policy Management API
  getKyvernoStatus: async () => {
    const response = await fetch(`${API_BASE}/api/admin/kyverno/status`);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  },

  installKyverno: async () => {
    const response = await fetch(`${API_BASE}/api/admin/kyverno/install`, {
      method: 'POST'
    });
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  ...
  getKyvernoPolicies: async () => {
    const response = await fetch(`${API_BASE}/api/admin/kyverno/policies`);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  },

  getKyvernoPolicy: async (policyId) => {
    const response = await fetch(`${API_BASE}/api/admin/kyverno/policies/${encodeURIComponent(policyId)}`);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  },

  updateKyvernoPolicy: async (policyId, config) => {
    const response = await fetch(`${API_BASE}/api/admin/kyverno/policies/${encodeURIComponent(policyId)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(config)
  ...
  getKyvernoViolations: async () => {
    const response = await fetch(`${API_BASE}/api/kyverno/violations`);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  },

  reconcileKyvernoPolicies: async () => {
    const response = await fetch(`${API_BASE}/api/admin/kyverno/reconcile`, {
      method: 'POST'
    });
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
