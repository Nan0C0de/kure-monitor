...
import { Plus, Trash2, AlertCircle, CheckCircle, Shield, Activity, Bot, Bell, EyeOff, Clock, Settings, Save, Globe } from 'lucide-react';
...
const AdminPanel = ({ isDark = false }) => {
  ...
  // Trusted Container Registries state
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [excluded, available, excludedPodsData, monitoredPodsData, excludedRulesData, ruleTitlesData, trustedRegistriesData, retentionData, ignoredRetentionData] = await Promise.all([
        ...
        api.getTrustedRegistries().catch(() => []),
      ...
      setTrustedRegistries(trustedRegistriesData);
  ...
  const handleAddRegistry = async () => {
    ...
    if (DEFAULT_REGISTRIES.includes(registry)) {
      setError('This registry is already trusted by default');
      return;
    }
