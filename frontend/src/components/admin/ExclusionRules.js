...
const ExclusionRules = ({ isDark, onError, onSuccess }) => {
  ...
  return (
    <div>
      ...
      <div className={`mb-4 p-4 border rounded-sm ${isDark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50/50'}`}>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              {showGlobalRuleSuggestions && filteredGlobalRuleSuggestions.length > 0 && (
                <div
                  className={`absolute z-10 w-full mt-1 border rounded-sm shadow-lg max-h-48 overflow-y-auto ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
        ...
        <form onSubmit={handleRuleSubmit}>
          <div className="flex gap-2">
            ...
            <select
              ...
              className={`px-3 py-2 text-sm border rounded-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${isDark ? 'bg-gray-700 border-gray-600 text-gray-200' : 'bg-white border-gray-300 text-gray-900'}`}
            >
