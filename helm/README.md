...
| `agent.image.tag` | Agent image tag | `2.3.4` |
| `securityScanner.image.tag` | Security scanner image tag | `2.3.4` |
| `backend.replicaCount` | Backend replica count | `1` |
| `backend.image.tag` | Backend image tag | `2.3.4` |
| `frontend.image.tag` | Frontend image tag | `2.3.4` |
...
```bash
helm repo update
helm upgrade kure-monitor kure-monitor/kure --version 2.3.4 -n kure-system
```
