...
class PodScanner:
    ...
    async def scan_single_pod(self, pod):
        ...
        # Refresh trusted registries ONCE per pod scan (was per-container).
        # The refresh is itself throttled by ExclusionManager (1 min interval),
        # but hoisting avoids N calls + locking per pod.
        await self.scanner.exclusion_mgr.refresh_trusted_registries()
        trusted_registries = set(TRUSTED_REGISTRIES) | set(
            self.scanner.exclusion_mgr.admin_trusted_registries
        )
        ...
        if pod.spec.volumes:
            for volume in pod.spec.volumes:
                if volume.host_path:
                    severity = (
                        "critical"
                        if volume.host_path.path
                        in ["/", "/etc", "/var", "/root", "/home"]
                        else "high"
                    )
                    await self.scanner.report_finding(
                        {
        ...
        for container in all_containers:
            ...
            if (
                not sec_ctx
                or sec_ctx.allow_privilege_escalation is None
                or sec_ctx.allow_privilege_escalation
            ):
                await self.scanner.report_finding(
                    {
                        "resource_type": "Pod",
            ...
            caps_dropped_all = (
                sec_ctx
                and sec_ctx.capabilities
                and sec_ctx.capabilities.drop
                and (
                    "ALL" in sec_ctx.capabilities.drop
                    or "all" in sec_ctx.capabilities.drop
                )
            )
            ...
            # Check for untrusted registry (uses cached set hoisted above)
            image_registry = get_image_registry(image)
            if image_registry and image_registry not in trusted_registries:
                await self.scanner.report_finding(
                    {
                        "resource_type": "Pod",
                        "resource_name": pod_name,
            ...
            if (
                not container.image_pull_policy
                or container.image_pull_policy == "IfNotPresent"
            ):
                if image.endswith(":latest") or (":" not in image.split("/")[-1]):
                    await self.scanner.report_finding(
                        {
        ...
        for container in all_containers:
            container_name = container.name
            apparmor_key = (
                f"container.apparmor.security.beta.kubernetes.io/{container_name}"
            )
    ...
    async def scan_pods(self):
        ...
        try:
            ...
            for pod in pods.items:
                if self.scanner.exclusion_mgr.is_namespace_excluded(
        ...
        try:
            ...
            for pod in pods.items:
                ...
                if automount is None or automount:
                    try:
                        sa = self.scanner.v1.read_namespaced_service_account(
                            sa_name, namespace
                        )
                ...
                if sa_namespace == "kube-system" or (
                    namespace != "kube-system" and sa_name.startswith("system:")
                ):
                    await self.scanner.report_finding(
                        {
                            "resource_type": "Pod",
    ...
    async def scan_seccomp_profiles(self):
        ...
        try:
            ...
            for pod in pods.items:
                ...
                pod_has_seccomp = (
                    pod_sec_ctx
                    and pod_sec_ctx.seccomp_profile
                    and pod_sec_ctx.seccomp_profile.type
                    in ["RuntimeDefault", "Localhost"]
                )

                all_containers = (pod.spec.containers or []) + (
                ...
                for container in all_containers:
                    ...
                    container_has_seccomp = (
                        sec_ctx
                        and sec_ctx.seccomp_profile
                        and sec_ctx.seccomp_profile.type
                        in ["RuntimeDefault", "Localhost"]
                    )
