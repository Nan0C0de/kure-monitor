...
class ResourceScanner:
    ...
    async def scan_single_deployment(self, deployment):
        ...
        await self.scanner.backend_client.delete_findings_by_resource(
            "Deployment", namespace, deploy_name
        )
        ...
        if replicas >= 2:
            ...
            has_anti_affinity = (
                ...
                and (
                    ...
                    or affinity.pod_anti_affinity.preferred_during_scheduling_ignored_during_execution
                )
            )
    ...
    async def scan_single_cluster_role(self, role):
        ...
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.scanner._set_resource_context(
            role, "rbac.authorization.k8s.io/v1", "ClusterRole"
        )

        await self.scanner.backend_client.delete_findings_by_resource(
            "ClusterRole", "cluster-wide", role_name
        )
        ...
        if role.rules:
            for rule in role.rules:
                ...
                if "secrets" in resources:
                    dangerous_verbs = [
                        v for v in verbs if v in ["get", "list", "watch", "*"]
                    ]
                    if dangerous_verbs:
                        await self.scanner.report_finding(
                            {
    ...
    async def scan_single_cronjob(self, cronjob):
        ...
        all_containers = (job_template.containers or []) + (
            job_template.init_containers or []
        )
    ...
    async def scan_network_policies(self):
        ...
        try:
            ...
            network_policies = (
                self.scanner.networking_v1.list_network_policy_for_all_namespaces()
            )
    ...
    async def scan_cluster_role_bindings(self):
        ...
        try:
            ...
            for binding in bindings.items:
                ...
                self.scanner._set_resource_context(
                    binding, "rbac.authorization.k8s.io/v1", "ClusterRoleBinding"
                )
            ...
            logger.error(
                f"Kubernetes API error while scanning ClusterRoleBindings: {e}"
            )
        except Exception as e:
    ...
    async def scan_pod_disruption_budgets(self):
        ...
        try:
            deployments = self.scanner.apps_v1.list_deployment_for_all_namespaces()
            pdbs = (
                self.scanner.policy_v1.list_pod_disruption_budget_for_all_namespaces()
            )
            timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
            ...
            for deployment in deployments.items:
                ...
                if namespace in pdb_selectors:
                    for pdb_labels in pdb_selectors[namespace]:
                        if all(
                            deploy_labels.get(k) == v for k, v in pdb_labels.items()
                        ):
                            has_pdb = True
                            break
                ...
                for key, value in data.items():
                    ...
                    for sensitive_key in sensitive_keys:
                        if sensitive_key in key_lower:
                            found_sensitive.add(
                                f"key '{key}' (contains '{sensitive_key}')"
                            )
                            break
                    ...
                    if value:
                        for pattern, pattern_name in sensitive_patterns:
                            if re.search(pattern, value, re.IGNORECASE):
                                found_sensitive.add(
                                    f"value matching '{pattern_name}' pattern"
                                )
                                break
        ...
        try:
            ...
            for pv in pvs.items:
                ...
                if pv.spec.host_path:
                    ...
                    severity = (
                        "critical"
                        if host_path in ["/", "/etc", "/var", "/root", "/home"]
                        else "high"
                    )
                    await self.scanner.report_finding(
                        {
