...
class DismissDeletedPodRequest(BaseModel):
    """Agent-side notification that a pod was deleted from K8s; auto-resolves
    any matching open failures. Bounded string lengths align with the
    K8s DNS-1123 subdomain limit (253 chars)."""

    namespace: str = Field(..., min_length=1, max_length=253)
    pod_name: str = Field(..., min_length=1, max_length=253)


class RetentionMinutesRequest(BaseModel):
    """Auto-delete retention window in minutes. 0 disables; max 30 days."""
...
def create_pod_ingest_router(deps: RouterDeps) -> APIRouter:
    ...
    async def report_failed_pod(report: PodFailureReport):
        ...
        """
        logger.info(
            f"Received failure report for pod: {report.namespace}/{report.pod_name}"
        )

        try:
            if not report.pod_name or not report.namespace:
                raise HTTPException(
                    status_code=400, detail="Pod name and namespace are required"
                )
            ...
            logger.info(
                f"Saving pod failure to database: {report.namespace}/{report.pod_name}"
            )
            ...
            if logs_eligible:
                try:
                    log_rows_written = await db.save_pod_failure_logs(
                        pod_id, report.failure_logs
                    )
                    if log_rows_written:
            ...
            logger.info(
                f"Broadcasting pod failure via WebSocket: {report.namespace}/{report.pod_name}"
            )
            ...
            logger.info(
                f"Successfully processed failed pod: {report.namespace}/{report.pod_name}"
            )
            return response
            ...
            raise HTTPException(
                status_code=500,
                detail="Internal server error while processing pod failure",
            )

    @router.post("/pods/dismiss-deleted")
    async def dismiss_deleted_pod(request: DismissDeletedPodRequest):
        ...
        try:
            ...
            logger.info(
                f"Auto-resolved pod: {namespace}/{pod_name} ({len(resolved_pods)} records)"
            )
...
def create_pod_router(deps: RouterDeps) -> APIRouter:
    ...
    async def get_ignored_pods():
        ...
        try:
            return await db.get_pod_failures(
                include_dismissed=True, dismissed_only=True
            )
        except Exception as e:
    ...
    async def dismiss_pod_failure(pod_id: int):
        ...
        try:
            ...
            if pod_failure and notification_service:
                await notification_service.send_pod_resolved_notification(
                    namespace=pod_failure.namespace, pod_name=pod_failure.pod_name
                )
        ...
        try:
            ...
            if updated:
                ...
                if request.status in ("resolved", "ignored") and notification_service:
                    await notification_service.send_pod_resolved_notification(
                        namespace=updated.namespace, pod_name=updated.pod_name
                    )
    ...
    @router.post(
        "/pods/failed/{pod_id}/retry-solution",
        response_model=PodFailureResponse,
        dependencies=[Depends(require_write)],
    )
    async def retry_ai_solution(pod_id: int):
        ...
        try:
            ...
            container_statuses = [
                ContainerStatus(**s) if isinstance(s, dict) else s
                for s in pod_failure.container_statuses
            ]
            events = [
                PodEvent(**e) if isinstance(e, dict) else e for e in pod_failure.events
            ]
            ...
            logger.info(
                f"Successfully regenerated AI solution for pod: {pod_failure.namespace}/{pod_failure.pod_name}"
            )
            return updated_pod
    ...
    async def troubleshoot_pod(pod_id: int, regenerate: bool = False):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to generate log-aware troubleshoot solution",
            )
    ...
    async def delete_pod_record(pod_id: int):
        ...
        try:
            ...
            logger.info(
                f"Deleted pod record: {pod_failure.namespace}/{pod_failure.pod_name} (id={pod_id})"
            )
    ...
    @router.put(
        "/admin/settings/history-retention", dependencies=[Depends(require_write)]
    )
    async def set_history_retention(request: RetentionMinutesRequest):
        """Set the history auto-delete retention (minutes). 0 = disabled. Max 43200 (30 days)."""
    ...
    @router.put(
        "/admin/settings/ignored-retention", dependencies=[Depends(require_write)]
    )
    async def set_ignored_retention(request: RetentionMinutesRequest):
        """Set the ignored pods auto-delete retention (minutes). 0 = disabled. Max 43200 (30 days)."""
        minutes = request.minutes
        try:
